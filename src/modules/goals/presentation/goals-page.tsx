  ...
  return (
    <div className="pb-16">
      <header className="rise-in mb-8">
        <p className="island-kicker mb-2">{m.goals_kicker()}</p>
      ...
      <section className="mt-10" aria-labelledby="goals-list-heading">
        ...
        >
          {m.goals_list_heading()}
        </h2>

        {goals.goals.length === 0 ? (
          <Card className="island-shell rounded-2xl border-[var(--border-default)] py-8 shadow-none">
            <CardHeader className="text-center">
              <CardTitle className="display-title text-xl">{m.goals_empty_title()}</CardTitle>
              <CardDescription>{m.goals_empty_description()}</CardDescription>
            </CardHeader>
          </Card>
        ) : (
