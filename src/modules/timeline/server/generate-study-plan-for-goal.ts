...
export type GenerateStudyPlanForGoalInput = z.infer<typeof generateStudyPlanInputSchema>;

export const generateStudyPlanForGoalFn = createServerFn({ method: "POST" })
  .inputValidator((input: GenerateStudyPlanForGoalInput) =>
    generateStudyPlanInputSchema.parse(input),
  )
  .handler(async ({ data }): Promise<StudyPlan> => {
    return Sentry.startSpan({ name: "generateStudyPlanForGoal" }, async () => {
      const authResult = await requireAuthenticatedAppUser();
      if (!authResult.ok) {
        throw new Error(identityErrorMessage(authResult.error));
      }

      const result = await getTimelineModule().generateStudyPlanForGoal.execute({
