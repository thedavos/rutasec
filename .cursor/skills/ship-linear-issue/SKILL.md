...
description: >-
  ...
  GitLab MR to main (glab), Linear In Progress/Done, vp check/tests, worktree
...
4. **Linear:** `In Progress` when work starts; `Done` when the merge request **merges** to `main` (not merely when the MR is opened).
5. **Finish:** `vp check`, `vp test run` (or `npm run test:coverage` if the change touches scoped modules), GitLab MR to `main`, remove worktree.
6. **No GitHub PR.** `origin` is `gitlab.com:davos/rutasec`. Use `glab`, not `gh`.
7. If the issue or plan calls for a local test account, follow `db/README.md` and `npm run db:smoke-user:local`.
8. **Docs and agent guidance:** when the change affects how humans or agents work in the repo, update the relevant files in the same MR (see step 5).
...
- [ ] 7. Commit, push origin, glab mr create → main
- [ ] 8. Worktree remove + prune (from primary repo)
- [ ] 9. Linear: Done after MR merges; link MR on issue if useful
```
...
Before opening the MR, check whether the issue changed operational or agent-facing knowledge. Skip if nothing new applies; do not pad docs for small fixes.
...
glab mr create --target-branch main --title "<title>" --description "<summary + test plan + Closes DAV-XXX>"
```

Return the MR URL. Babysit GitLab CI on the MR if pipelines exist.

### 8. Worktree cleanup

From the **primary** repo (after MR exists):
...
Keep the remote branch for review until the MR merges.

### 9. Linear Done

When the MR is **merged** to `main`:

```text
save_issue(id: "<DAV-XXX>", state: "Done")
```

Optionally attach the MR link via `links` on `save_issue`.
...
- GitLab plugin: `create-merge-request` command if MCP is preferred over CLI
